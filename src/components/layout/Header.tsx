...
export function Header() {
  ...
  return (
    <header
      className={cn(
        "fixed top-0 left-0 right-0 z-50 transition-all duration-300 backdrop-blur-xl bg-bg/80",
        scrolled
    ...
    >
          ...
          {siteConfig.nav.map((item) =>
            item.children ? (
              ...
              >
                ...
                {dropdownOpen && (
                  <div className="absolute left-0 top-full mt-1 min-w-[200px] rounded-xl border border-white/10 bg-bg/95 backdrop-blur-xl p-2">
      ...
      {/* Mobile nav */}
      {mobileOpen && (
        <div className="fixed inset-0 top-0 flex flex-col items-center justify-center gap-6 bg-bg/95 backdrop-blur-xl lg:hidden">
          {siteConfig.nav.map((item) =>
            item.children ? (
