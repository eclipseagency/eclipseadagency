...
/* ─────────────────────────────────────────────────────────
   Shared pointer state — no React state = zero re-renders
   ───────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────
   Camera rig — subtle mouse-driven parallax
   ───────────────────────────────────────────────────────── */
function CameraRig() {
...
/* ─────────────────────────────────────────────────────────
   Eclipse Orb — dark sphere with GLSL fresnel corona
   ───────────────────────────────────────────────────────── */
function EclipseOrb() {
  ...
  useFrame(({ clock }) => {
    ...
    if (groupRef.current) {
      groupRef.current.rotation.y +=
        (pointer.sx * 0.4 - groupRef.current.rotation.y) * 0.015;
      groupRef.current.rotation.x +=
        (-pointer.sy * 0.25 - groupRef.current.rotation.x) * 0.015;
    }

    coronaMat.uniforms.uTime.value = t;
    coronaMat.uniforms.uIntensity.value =
      1 + Math.sqrt(pointer.sx ** 2 + pointer.sy ** 2) * 0.5;

    if (outerRef.current) {
      outerRef.current.scale.setScalar(1.35 + Math.sin(t * 0.5) * 0.06);
    }
  });
...
/* ─────────────────────────────────────────────────────────
   Interactive Particle Field — mouse repulsion + spring
   ───────────────────────────────────────────────────────── */
function ParticleField({ count = 1200 }: { count?: number }) {
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      fragmentShader: /* glsl */ `
        ...
        void main() {
          ...
          float fogFade = smoothstep(12.0, 3.0, vDist);
          alpha *= fogFade;
  ...
  useFrame(({ clock }) => {
    ...
    const mx = pointer.sx * 3.5;
    const my = pointer.sy * 2.5;
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Damping
      velocities[i3] *= 0.955;
      velocities[i3 + 1] *= 0.955;
      velocities[i3 + 2] *= 0.955;
...
/* ─────────────────────────────────────────────────────────
   Orbital Rings — mouse-reactive torus rings
   ───────────────────────────────────────────────────────── */
  ...
  const configs = useMemo(
    () => [
      ...
      },
      {
        r: 2.3,
        tube: 0.003,
        tilt: [-Math.PI / 4, Math.PI / 6, 0] as const,
        speed: -0.08,
        opacity: 0.07,
      },
      {
        r: 3.0,
        tube: 0.003,
        tilt: [0, Math.PI / 5, Math.PI / 8] as const,
        speed: 0.05,
        opacity: 0.04,
      },
    ],
...
/* ─────────────────────────────────────────────────────────
   Nebula dust — large slow-drifting ambient particles
   ───────────────────────────────────────────────────────── */
function NebulaDust() {
  ...
  const { geometry, material } = useMemo(() => {
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      vertexShader: /* glsl */ `
        ...
        void main() {
          vec3 p = position;
          p.x += sin(uTime * 0.08 + position.y * 0.5) * 0.3;
          p.y += cos(uTime * 0.06 + position.x * 0.3) * 0.2;

          vec4 mv = modelViewMatrix * vec4(p, 1.0);
          gl_Position = projectionMatrix * mv;
          gl_PointSize = aSize * uPixelRatio * (60.0 / -mv.z);
          gl_PointSize = clamp(gl_PointSize, 1.0, 60.0);
          vAlpha = smoothstep(15.0, 4.0, -mv.z) * 0.08;
        }
      `,
      fragmentShader: /* glsl */ `
        varying float vAlpha;
        ...
        void main() {
          ...
          gl_FragColor = vec4(1.0, 0.55, 0.2, alpha);
        }
      `,
      transparent: true,
      blending: THREE.AdditiveBlending,
...
/* ─────────────────────────────────────────────────────────
   Main canvas export — dynamic imported in Hero (ssr:false)
   ───────────────────────────────────────────────────────── */
