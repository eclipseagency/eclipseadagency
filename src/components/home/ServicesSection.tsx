    ...
    <section id="services" className="relative pt-8 pb-16 md:pt-12 md:pb-24 overflow-hidden">
      ...
      <div className="relative mx-auto max-w-[1100px] px-5 md:px-8">
        <div className="text-center mb-10 md:mb-14">
          <p className="text-[10px] font-semibold uppercase tracking-[0.4em] text-[#ff6b35]/50 mb-3 md:text-xs" data-fade>{t("services.badge")}</p>
          <h2 className="font-heading text-[clamp(1.8rem,4vw,3.5rem)] font-bold text-white" data-reveal>{t("services.title")}</h2>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 md:gap-6">
          {servicesOverview.map((service, i) => {
            // Map service slug to translation key prefix
            const keyMap: Record<string, string> = {
              branding: "services.branding",
              "digital-marketing": "services.marketing",
              "web-apps": "services.web",
              animation: "services.animation",
              production: "services.production",
              "3d-creations": "services.3d",
            };
            const prefix = keyMap[service.slug];
            const title = prefix ? t(`${prefix}.title`) : service.title;
